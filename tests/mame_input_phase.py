...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="openaladdin-input-phase-") as name:
        trace = Path(name)
        environment = oa._clean_mame_environment()
        environment.update({
            ...
            "OPENALADDIN_ROM_SHA256": oa.hashes(oa.default_rom())["sha256"],
        })
        status = oa.run_shell_tool(
            "openaladdin/mame/run.sh", [str(oa.default_rom())], env=environment
        )
        if status != 0:
            return status
        oa.synchronize_state_trace(trace)
        _, states, _ = oa.load_state_trace(trace / "state.jsonl")
