...
def main() -> int:
    ...
    if not headless.is_file():
        raise SystemExit(f"Ghidra launcher not found: {headless}; run tools/openaladdin/ghidra/setup.py")
    if not (project_dir / f"{project_name}.gpr").exists() and not (project_dir / f"{project_name}.rep").exists():
        raise SystemExit(f"Ghidra project not found: {project_dir}; run tools/openaladdin/ghidra/import_rom.py first")
