...
def _run_scenario(
    ...
    command = [str(ROOT / "tools/mame-trace.sh"), str(rom)]
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "re/mame/capture_matrix.yml",
    ...
    if not loader_path.is_file():
        raise SystemExit(f"RNC loader analysis not found: {loader_path}; run tools/extract-assets.py first")
    output_root = args.output.resolve()
    output_root.mkdir(parents=True, exist_ok=True)
    scenarios = _load_scenarios(config_path, set(args.scenario))
    parser_path = ROOT / "tools/analyze-rnc-load-trace.py"
    manifest = {
    ...
    subprocess.run([
        sys.executable,
        str(ROOT / "tools/merge-mame-traces.py"),
        str(matrix_path),
        "--output",
        str(combined),
    ], cwd=ROOT, check=True)
    ...
    subprocess.run([
        sys.executable,
        str(ROOT / "tools/analyze-rnc-runtime.py"),
        "--trace",
        str(combined),
        "--load-trace",
        str(combined / "rnc_loads.json"),
