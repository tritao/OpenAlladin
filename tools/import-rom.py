...
def resolve_rom(requested: Path | None, expected: dict) -> Path:
    ...
    candidates.extend(
        [
            ROOT / str(expected.get("expected_filename", "Disneys_Aladdin_U_p1.bin")),
            ROOT / "rom" / str(expected.get("expected_filename", "aladdin-usa.bin")),
        ]
    )
...
def main() -> int:
    ...
    verifier = ROOT / "tools/verify-rom.py"
    ...
    if not headless.is_file():
        raise SystemExit(f"Ghidra is not installed. Run: python tools/setup-ghidra.py\nMissing: {headless}")
