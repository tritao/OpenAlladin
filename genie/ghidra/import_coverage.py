...
from genie.common import ROOT, load_yaml
...
def main() -> int:
    ...
    if not coverage.is_file():
        raise SystemExit(f"coverage report not found: {coverage}; run genie coverage merge")
    ...
    if not project_marker.is_file() and not project_rep.is_dir():
        raise SystemExit(f"Ghidra project not found: {project_dir}; run genie ghidra rebuild first")

    headless = ROOT / config["ghidra"]["install_dir"] / "support/pyghidraRun"
    if os.name == "nt":
        headless = headless.with_suffix(".bat")
    if not headless.is_file():
        raise SystemExit(f"Ghidra launcher not found: {headless}; run genie setup first")
