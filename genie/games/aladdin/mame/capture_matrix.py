...
from genie.common import ROOT, load_yaml, write_json
...
def _run_scenario(
    ...
    command = [str(ROOT / "genie/mame/run.sh"), str(rom)]
...
def main() -> int:
    ...
    if not loader_path.is_file():
        raise SystemExit(f"RNC loader analysis not found: {loader_path}; run genie assets first")
    output_root = args.output.resolve()
    output_root.mkdir(parents=True, exist_ok=True)
    scenarios = _load_scenarios(config_path, set(args.scenario))
    parser_path = ROOT / "genie/assets/rnc_load_trace.py"
    manifest = {
    ...
    subprocess.run([
        sys.executable,
        str(ROOT / "genie/mame/merge_traces.py"),
        str(matrix_path),
        "--output",
        str(combined),
    ], cwd=ROOT, check=True)
    ...
    subprocess.run([
        sys.executable,
        str(ROOT / "genie/assets/rnc_runtime_cli.py"),
        "--trace",
        str(combined),
        "--load-trace",
        str(combined / "rnc_loads.json"),
