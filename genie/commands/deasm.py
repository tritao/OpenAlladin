...
DEFAULT_METRICS = ROOT / "build/re/deasm/metrics.json"
...
def _write_metrics(path, build: _ExactBuild, *, source, rebuilt=None, rom_size=None) -> None:
    path = resolve(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        json.dumps(
            {
                "format": "openaladdin-deasm-metrics-v1",
                "rom_size": rom_size,
                "instructionized": build.instructionized,
                "raw_fallback": build.raw_fallback,
                "attempts": build.attempts,
                "source": str(source),
                "rebuilt": str(rebuilt) if rebuilt is not None else None,
            },
            indent=2,
            sort_keys=True,
        )
...
def command_deasm_build(args: argparse.Namespace) -> int:
    ...
    _write_metrics(
        getattr(args, "metrics", DEFAULT_METRICS),
        build,
        source=output,
...
def command_deasm_stats(args: argparse.Namespace) -> int:
    try:
        value = _input(args)
        metrics_path = resolve(getattr(args, "metrics", DEFAULT_METRICS))
        metrics = json.loads(metrics_path.read_text(encoding="utf-8")) if metrics_path.is_file() else None
        if metrics and metrics.get("rom_size") is not None and int(metrics["rom_size"]) != len(value.rom):
            metrics = None
        stats = collect(value, _symbols(), metrics=metrics)
...
def command_deasm_todo(args: argparse.Namespace) -> int:
    try:
