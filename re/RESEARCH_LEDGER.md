...
- `re/mame/findings/20260830-runtime-coverage-symbol-promotions-v1.json`
  ...
  multiple covered PCs in the imported whole-ROM coverage corpus, followed by
  a conservative extension for six single-PC functions observed across at
...
- `re/mame/findings/20260830-semantic-review-pair-runtime-audit-v1.json`
  records a bounded 300-frame breakpoint replay and 900-frame actor-init
  inventory from the compatible upper-response checkpoint. Neither the
  unindexed Type-5E/Type-0x84 producer nor any open template was selected, so
  that checkpoint is excluded from the next runtime campaign.
...
The adjacent presentation state is now named as well. `LEVEL08_VDP_SCROLL_OFFSET`
at `FFF0A2` is initialized to `0x140` by the Level-08 exit path, advanced by
the Level-08 event cadence, and consumed by that level's camera-scroll VDP
...
## Player terrain-brake AnimationVM producer (20260831)

The previously missing nonzero producer for `PLAYER_TERRAIN_BRAKE_STATE` at
`0x00FFF101` is now recovered from the AnimationVM command encoding. The
`ED 01 F101 0000` commands at `0x00121FDC` in
`PLAYER_ANIM_SPECIAL_CAMERA_RUN` and `0x0012200E` in `PLAYER_ANIM_RUN` clear
the latch, while the `ED 01 F101 0001` command at `0x00122078` in
`PLAYER_ANIM_RUN` arms it before the landing-response handoff. This explains
why the Ghidra absolute-reference inventory contained only clear writers: the
nonzero producer is an indirect VM stream write.

The absolute clear set is also bounded: seven writes belong to
`Player_TerrainResponseStateMachine`, one to `Actor_PlayerCollisionPass`, and
two to `Player_TerrainLandingResolver`; the only consumer is the settled-
landing test at `0x001AA10A`. The static contract is recorded in
...
| `20260830-low-confidence-scene-terrain-services-static-v1` | recorded-static-decompilation | Closed the remaining low-confidence function queue: trace-validated SceneScript_AdvanceState, decompiled Scene_EnterTransitionMode and SceneScript_CompleteToState1, and the exact Terrain_RenderViewportWithPhaseOffset name/body |
...
| `20260830-type84-type0f-child-movement-static-v1` | recorded-static-decompilation | Promoted the exact 66-byte Type-0x84 Type-0x0F child MovementVM response at 0x00120A00-0x00120A41 from the direct ED publication at 0x00123D8C, including its signed settle profile, player-X gate, random root loop, and terminal state clear |
| `20260830-scene-numeric-labels-static-v1` | recorded-static-disassembly | Bounded the exact ten-byte ASCII object at 0x00126EB6-0x00126EBF containing the NUL-terminated labels 30, 60, and 90 immediately after the scene-resource mode table; retained provisional confidence because its runtime consumer is unresolved |
| `20260830-fixed-width-blank-records-static-v1` | recorded-static-disassembly | Bounded the exact three-record, 48-byte FF/space/NUL corpus at 0x00126D4E-0x00126D7D after the live sound-test sentinel and before the scene-resource mode table; kept it separate from the live table with provisional confidence because no consumer is known |
| `20260830-type01-collision-response-helper-static-v1` | recorded-static-decompilation | Named the exact 52-byte collision-response helper at 0x001ABF9C-0x001ABFCF that optionally queues response audio 3, publishes actor type 0x84, installs ACTOR_ANIM_TYPE84_TYPE01_RESPONSE, and clears actor field +0x37 |
| `20260830-actor-terrain-complete-body-static-v1` | recorded-static-decompilation | Made the existing 730-byte Actor_TerrainCollisionLoop body canonical from 0x001ADB5C through 0x001ADE35, including the camera-relative comparison previously left as a Ghidra split gap before the shared response tail |
| `20260830-interaction-spawn-adjacent-variants-static-v1` | recorded-static-decompilation | Bounded and named the complete 0x001B7474-0x001B7493 runtime-type-0x45 and 0x001B74A0-0x001B74B1 runtime-type-0x5B interaction-spawn sibling bodies; retained unresolved selector/reachability because neither has a current static dispatch pointer |
| `20260830-actor-collision-reinitialization-variant-static-v1` | recorded-static-decompilation | Bounded and named the adjacent 32-byte collision-response lifecycle variant at 0x001ABFD0, which clears/releases A1 around the Type-0x2D interaction path, initializes A2 from ACTOR_TEMPLATE_COLLISION_RESPONSE, and adds response counter 0x000F; retained unresolved selector/reachability because no current static pointer reaches it |
| `20260830-interaction-spawn-unindexed-variant-static-v1` | recorded-static-decompilation | Bounded and named the complete 18-byte 0x001B6BFC interaction-spawn variant that reverse-spawns the Type-0x55 template and promotes the result to runtime type 0x57; retained unresolved selector/reachability because no current static pointer reaches it |
| `20260830-actor-template-unreferenced-records-static-v1` | recorded-static-disassembly | Promoted three exact 20-byte Type-0x84 actor-template records at 0x001B7990, 0x001B7A58, and 0x001B82DC from the compact initializer-record cadence; retained provisional field-based names and unresolved runtime reachability because no producer or consumer is known |
| `20260830-fixed-width-blank-records-menu-static-v1` | recorded-static-disassembly | Bounded the exact four-record, 64-byte FF/space/NUL corpus at 0x0012671E-0x0012675D between the options presentation stream and live sound-test table; kept it separate and provisional because no consumer is known |
| `20260830-unreferenced-animation-self-loop-static-v1` | recorded-static-disassembly | Promoted the exact eight-byte 0x00123DE2 AnimationVM frame/self-loop between the Type-0x0F child and Type-0x6E default roots; retained provisional confidence because no external actor-template pointer or producer is known |
| `20260830-rnc-boundary-padding-static-v1` | recorded-static-decompilation | Promoted 31 isolated zero alignment bytes between exact RNC payload extents across scene, level, character, and map resources, verifying each following RNC\\x01 header while leaving mixed-content gaps unresolved |
| `20260830-rnc-graphics-resource-static-v1` | recorded-static-decompilation | Named the remaining 23 exact RNC-compressed Genesis-tile resources across scene, transition, title, menu, camera, and rebuild paths from loader analysis and decoded tile geometry, retaining source-oriented names where a narrower identity is not established |
| `20260830-audio-scene-boundary-padding-static-v1` | recorded-static-decompilation | Promoted the two-byte zero gap at 0x0000695E before SCENE_RESOURCE_RANDOM_ACTOR_VARIANTS and the one-byte sample-data boundary at 0x001E56BF before Audio_AcquireZ80BankWindow, preserving exact resource extents |
| `20260830-actor-frame-phase-child-alignment-padding-static-v1` | recorded-static-decompilation | Promoted the single zero alignment byte at 0x000049A8 between ACTOR_VM_DISPATCH_TABLE and the odd-addressed ACTOR_FRAME_PHASE_CHILD_ANIMATION_TABLE, whose selected longword fields remain even-aligned |
| `20260830-unreferenced-palette-data-static-v1` | recorded-static-disassembly | Promoted the exact 16-word palette source at 0x00129312 and three-band palette bank at 0x00129A32-0x00129A91 as provisional unreferenced palette data, retaining neutral names until consumers are recovered |
| `20260830-unreferenced-sprite-vdp-control-words-static-v1` | recorded-static-disassembly | Promoted the adjacent 0x60000003/0x40000003 VDP control-word pair at 0x00001CB6-0x00001CBD after the known player-sprite control word, retaining provisional confidence because no consumer is recovered |
| `20260830-unreferenced-menu-control-record-static-v1` | recorded-static-disassembly | Promoted the 18-byte X-labelled record at 0x00004000 immediately before MENU_CONTROL_LAYOUT_TABLE as provisional reserved menu-layout data, retaining its unresolved runtime selector and purpose |
| `20260830-actor-collision-handler-alignment-nop-static-v1` | recorded-static-disassembly | Promoted the exact 68000 NOP at 0x001AC456 as confirmed alignment padding between the Type-0x1C actor-collision handler and the shared Type-0A/common response entry |
| `20260830-actor-type3a-return-tail-static-v1` | recorded-static-disassembly | Closed the exact 2-byte RTS branch target at 0x001AF262-0x001AF263 inside the Type-0x3A player-collision handler, extending its canonical owner beyond Ghidra's split range |
| `20260830-audio-command-wrapper-family-static-v1` | recorded-static-disassembly | Promoted the exact fixed-command audio wrapper family at 0x001E5908-0x001E5A23 plus the 0x001E58C2 command-05 wrapper, including the decompiled protected indexed Z80-result reader at 0x001E59F2 |
| `20260830-actor-linked-cleanup-helper-static-v1` | recorded-static-disassembly | Named the exact 22-byte linked-actor cleanup helper at 0x001ACFF8-0x001AD00D and corrected the runtime type-0x6E–0x73 handler family boundary to end at 0x001ACFF7 |
| `20260830-player-transition-flag-response-helper-static-v1` | recorded-static-disassembly | Named the exact decompiled 24-byte player transition-response setter at 0x001ADA98-0x001ADAAF, which installs 0x00121C62 and clears the player animation timer/gate while leaving direct reachability unresolved |
| `20260830-player-transition-flag-response-runtime-attribution-v1` | recorded-negative-runtime-audit | Confirmed that the natural replay's apparent PC=0x001ADA98 gate writes are post-instruction attribution for Player_TerrainLandingResolver at 0x001ADA90; the same trace observes the gate-clear writer inside Player_ResetAnimationResponseGate at 0x001AD170, which is now trace_validated, while an exact 0x001ADA98 entry breakpoint remains unhit and that isolated transition-response helper stays decompiled |
| `20260830-menu-palette-reachability-v1` | recorded-runtime-audit | Validated one startup execution of Menu_RunOptionsAndSelectNextState at 0x001B3B96 and recorded a negative 900-frame idle result for its 0x001B3C7C continuation, leaving MENU_OPTIONS_PALETTE_BANK_128F52 runtime reachability unresolved |
| `20260830-function-queue-confidence-closure-v1` | recorded-static-decompilation | Promoted the exact audio Z80 indexed-result reader and player transition-response setter from provisional to decompiled confidence; their low-level bodies are understood even though the former has no current caller and the latter has no recovered direct producer |
| `20260830-player-terrain-bounce-state-static-v1` | recorded-static-decompilation | Named the complete 114-byte player terrain-bounce velocity/state advancement helper at 0x001ADAB0-0x001ADB21, including its gate checks, capped 0x0078 acceleration, state-0x28 animation restart, and boundary before the separate landing-state branch target |
| `20260830-player-terrain-landing-state-tail-static-v1` | recorded-static-disassembly | Closed the exact 8-byte internal landing-state tail at 0x001ADB22-0x001ADB29, a direct branch target of Player_TerrainLandingResolver that sets TERRAIN_LANDING_STATE and returns |
| `20260830-scene-resource-service-gap-closures-static-v1` | recorded-static-decompilation | Named three complete scene-resource VDP/service bodies previously hidden in UNKNOWN gaps at 0x001B4836-0x001B4895, 0x001B4A96-0x001B4B27, and 0x001B4F04-0x001B4F7B, while retaining the adjacent RTS stub and direct-selector reachability as unresolved |
| `20260830-unreferenced-movement-streams-static-v1` | recorded-static-disassembly | Promoted exact provisional MovementVM owners for the former 0x0011F800-0x0011F8A3 grid-response gap and 0x001203E8-0x001203F1 self-loop gap, while retaining both external producers and runtime reachability as unresolved |
| `20260830-unreferenced-scene-palette-data-static-v1` | recorded-static-disassembly | Split the remaining menu/scene presentation gaps into an exact provisional 0x09/0x00 tile-base stream at 0x00128E4B, one confirmed alignment byte at 0x00128EB1, a provisional 16-word 0x0EEE palette source at 0x00128EB2, and a provisional four-band palette bank at 0x00128F52, retaining all runtime consumers as unresolved |
| `20260830-startup-checksum-spin-static-v1` | recorded-static-disassembly | Promoted the exact 26-byte non-returning ROM-word accumulator and self-spin at 0x00000200-0x00000219, immediately before the reset-vector entry, while retaining its checksum consumer and live reachability as unresolved |
| `20260830-startup-bootstrap-payload-static-v1` | recorded-static-disassembly | Promoted the reset-consumed copied-payload suffix at 0x000002DE-0x000002E7 and post-copy hardware values at 0x000002E8-0x000002F7, while retaining the unconsumed 0x000002F8-0x00000313 tail for a later bootstrap investigation |
| `20260830-scene-script-prefix-static-v1` | recorded-static-disassembly | Promoted the exact four-byte zero alignment padding at 0x0000407E-0x00004081 before INITIAL_SCENE_SCRIPT, preserving the live startup cursor boundary at 0x00004082 |
| `20260830-unreferenced-fixed-record-bank-1f82-static-v1` | recorded-static-disassembly | Promoted the exact provisional sixteen-record, 160-byte opaque bank at 0x00001F82-0x00002021 between the actor-collision pointer table and transition-plane padding, without assigning an unsupported consumer or format |
| `20260830-reset-bootstrap-unconsumed-tail-static-v1` | recorded-static-disassembly | Promoted conservative opaque owners for the unconsumed 28-byte startup tail at 0x000002F8-0x00000313 and the two-byte word skipped at 0x00000334-0x00000335, retaining both roles as unresolved |
| `20260830-unreferenced-fixed-word-bank-6744-static-v1` | recorded-static-disassembly | Promoted the exact provisional 125-word opaque bank at 0x00006744-0x0000683D between the state-4 scene-resource streams and terrain surface-flags table, without assigning a consumer or narrower format |
| `20260830-gap-code-islands-static-v1` | recorded-static-decompilation | Promoted six exact code islands previously emitted as UNKNOWN: three initial-scene palette/VRAM helper variants at 0x001B25AC/0x001B25DA/0x001B2618, two palette-transition setup variants at 0x001B28DC/0x001B28F4, and the transition-counter ones-digit wrapper at 0x001B34AE |
| `20260830-actor-frame-phase-child-repeated-tail-static-v1` | recorded-static-disassembly | Promoted the exact 23-byte repeated phase-child table prefix at 0x000049C1-0x000049D7 as a provisional opaque owner, preserving the final unused zero as the shared leading byte of SCENE_RESOURCE_COMMAND_TABLE at 0x000049D8 |
| `20260830-orphaned-code-islands-static-v1` | recorded-static-disassembly | Promoted nine conservative provisional function owners for the final structurally decodable UNKNOWN bytes: seven isolated RTS stubs and two Ghidra-unreachable menu code islands, preserving physical ROM ownership without asserting live reachability |
| `20260830-menu-continuation-semantic-names-v1` | recorded-static-decompilation | Replaced the physical-only Menu_UnreachableCodeIsland labels at 0x001B3C5E and 0x001B3C7C with Menu_RunOptionsFrameLoopPrelude and Menu_RunOptionsPresentationContinuation, tying both ranges to Menu_RunOptionsAndSelectNextState while retaining conservative live-entry confidence |
| `20260830-function-boundary-closures-static-v1` | recorded-static-decompilation | Closed nine level camera-scroll callbacks and three terminal/shared service bodies at their exact Ghidra/ROM boundaries, promoted the documented rebuild and initial-C000 continuations, and reduced generated deasm mechanical labels from 16 to 2 |
| `20260830-deasm-split-fragment-labels-v1` | tooling-validation | Preserved non-automatic semantic Ghidra function names on split layout fragments, eliminating the final two generated mechanical labels without borrowing automatic Ghidra names or changing ROM ownership |
| `20260830-static-function-confidence-closure-v1` | recorded-static-decompilation | Promoted the exact checksum/spin body and the two menu continuations to decompiled confidence while retaining explicit unresolved reachability limits; only seven isolated RTS stubs remain low-confidence queue items |
| `20260830-menu-unreachable-continuation-palette-consumer-static-v1` | recorded-static-decompilation | Recovered the static menu-continuation consumer at 0x001B3CB4 for the former UNREFERENCED_PALETTE_BANK_128F52, renamed it MENU_OPTIONS_PALETTE_BANK_128F52, and retained the containing continuation's unresolved live reachability |
| `20260830-vm-stream-confidence-closure-v1` | recorded-static-decompilation | Promoted the exact decoded MovementVM grid/self-loop streams at 0x0011F800 and 0x001203E8 plus the AnimationVM self-loop at 0x00123DE2 to decompiled confidence, retaining unresolved external producers and runtime reachability |
| `20260830-scene-resource-actor-spawn-coordinates-static-v1` | recorded-static-decompilation | Recovered the menu-continuation installation of 0x00006744 into SCENE_RESOURCE_ACTOR_RECORD_CURSOR and promoted the former opaque bank to 62 scene-resource actor X/Y pairs plus a zero sentinel, while retaining unresolved live reachability |
| `20260830-orphaned-rts-stub-confidence-static-v1` | recorded-static-disassembly | Promoted the seven exact two-byte RTS owners at 0x001B0020, 0x001B0042, 0x001B0044, 0x001B1CFE, 0x001B4624, 0x001B4F02, and 0x001B634C to decompiled body confidence while retaining their higher-level roles and reachability as unresolved |
...
| `20260830-semantic-review-pair-runtime-audit-v1` | recorded-runtime-negative-evidence | Exercised the compatible upper-response actor-pair checkpoint for 300 breakpoint frames and 900 actor-initializer frames; the unindexed Type-5E/Type-0x84 producer and open templates were not selected, narrowing the next probe to later-level or presentation selectors |
...
| `20260830-expanded-runtime-coverage-confidence-v2` | recorded-runtime-evidence | Promoted twelve already-decompiled AnimationVM, camera, interaction, level-camera, and player-collision functions to trace_validated from repeated multi-scenario archive coverage; retained narrower two-scenario observations as decompiled |
| `20260830-expanded-runtime-coverage-confidence-v3` | recorded-runtime-evidence | Promoted ten additional already-decompiled AnimationVM, player, camera, render, and lifecycle functions to trace_validated from two-scenario archive coverage; no names or ROM ownership changed |
| `20260830-expanded-runtime-coverage-confidence-v4` | recorded-runtime-evidence | Replayed the long traversal and special-terrain routes under fresh scenario names, then promoted ActorVM_WriteActorOrRamValue, Frame_RunServicePass, and Actor_AllocateSpriteVRAM to trace_validated; retained the one remaining single-scenario observation |
| `20260830-orphaned-actor-variant-closure-static-v1` | recorded-static-negative-reachability | Closed six complete actor/player helper bodies as canonical unreferenced variants after confirming their exact contracts while finding no exported selector, incoming xref, or merged-runtime entry sample |
| `20260830-unreferenced-actor-vm-template-closure-static-v1` | recorded-static-negative-reachability | Closed seven complete actor VM/template fragments as canonical unreferenced variants, including the duplicate phase-child prefix and the Type-0x7C/0x0F22 template-linked streams, while retaining the four genuinely open data review targets |
| `20260830-unreferenced-anchor-response-movement-closure-static-v1` | recorded-static-negative-reachability | Closed the 624-byte Type-0x5E/Type-0x84 anchor-response MovementVM bank after its only known producer was classified as an unreferenced complete variant with no exported selector or runtime sample |
| `20260830-unreferenced-text-and-scene-stream-closure-static-v1` | recorded-static-negative-reachability | Closed the 30/60/90 text object and princess response command stream as fully decoded unreferenced static assets after finding no exported consumer or runtime sample |
| `20260830-menu-palette-selection-runtime-audit-v1` | recorded-runtime-negative-evidence | Entered the primary options index-1 branch with a deterministic down+B probe, confirmed the alternate secondary-options loop, and observed no execution of the 0x001B3CB4 palette load or ROM read from MENU_OPTIONS_PALETTE_BANK_128F52 |
...
| `20260830-scene-resource-tile-base-static-v1` | recorded-static-decompilation | Named SCENE_RESOURCE_TILE_BASE at FFEFF0 and closed its exact 16-bit contract: scene-resource commands 0x08-0x0B select the four VRAM tile bases, VDP_WriteTileWord ORs the selected base into tile words, and transition setup selects 0x4000 |
| `20260830-interaction-response-elapsed-ticks-static-v1` | recorded-static-decompilation | Named ACTOR_RESPONSE_ELAPSED_TICKS at FFEFEA and closed the exact response-lifecycle accumulator: Interaction_DrainResponseCounter advances it while ACTOR_RESPONSE_COUNTER is active, applies difficulty thresholds 5000/7500/10000, increments GAME_DIFFICULTY_COUNTER at the threshold, and clears the word |
| `20260830-actor-sprite-payload-count-static-v1` | recorded-static-decompilation | Named ACTOR_SPRITE_PAYLOAD_COUNT at FFEFEE and closed the renderer count contract: animation pass setup clears it, Render_BuildActorSpritePayload increments its low byte per 14-byte payload record, and Render_SubmitActorSprites transfers exactly the counted records; FFEFEF is retained as the low-byte access |
| `20260830-palette-transition-step-static-v1` | recorded-static-decompilation | Named PALETTE_TRANSITION_STEP at FFEFF6 and closed the render transition contract: Render_RunProgressivePaletteTransition clears and increments it across 47 VBlank updates, while Render_ConvergePaletteWordsByProgress consumes its current value for palette convergence |
| `20260830-level09-type50-spawn-cooldown-static-v1` | recorded-static-decompilation | Named LEVEL09_TYPE50_SPAWN_COOLDOWN at FFF118 and closed its exact callback cadence: Level09_EnterRoutine increments it every callback, wraps at 0x50, and uses the wrapped phase to gate the coordinate-triggered Type-0x50 spawn while the separate Type-0x52 path uses FRAME_PHASE_COUNTER |
| `20260830-level00-type13-palette-delay-static-v1` | recorded-static-decompilation | Named LEVEL00_TYPE13_PALETTE_DELAY at FFF124 and closed its exact eight-callback contract: Type-0x13 player/actor collision response reloads it to 8, Level00_EnterRoutine decrements it, and expiry uploads palette band 2 |
| `20260830-scene-state8-transition-count-static-v1` | recorded-static-decompilation | Named SCENE_STATE8_TRANSITION_COUNT at FFF006 and closed its exact state-8 contract: Level_ExitAndTerminalTransition increments it on terminal completion, stops after count 3, and SceneResource_RunStatePresentation uses the same threshold for its alternate resource load |
| `20260830-scene-resource-rebuild-phase-state-static-v1` | recorded-static-decompilation | Named SCENE_RESOURCE_REBUILD_PHASE_DELAY, SCENE_RESOURCE_REBUILD_WAIT_GATE, and SCENE_RESOURCE_REBUILD_PHASE_PENDING at FFF121/FFF120/FFF11E and closed their accesses to the SceneResource_RebuildAfterInteraction phase-delay, wait-release, and finalization paths |
| `20260830-scene-resource-fast-path-static-v1` | recorded-static-decompilation | Named SCENE_RESOURCE_FAST_PATH at FFF11B and closed its exact transition-service contract: Render_RunPaletteTransitionFrom selects Frame_RunServicePass when set or Frame_WaitForVBlankWork when clear, with startup clear and menu presentation set/clear sites |
| `20260830-global-prng-state-static-v1` | recorded-static-decompilation | Promoted GLOBAL_PRNG_STATE at FF7DEA and unified the random-step evidence: startup/options seed 0x00BC614E, and the scaled and scene-state-5 helpers update the shared 32-bit state with state = state*13 + 7 |
| `20260830-palette-transition-source-static-v1` | recorded-static-decompilation | Promoted PALETTE_TRANSITION_SOURCE at FF7DF2 and closed its four-band pointer contract: both palette-transition services publish A0 there and consume four contiguous 16-word ROM palette bands |
| `20260830-game-runtime-mode-static-v1` | recorded-static-decompilation | Promoted GAME_RUNTIME_MODE at FF7E20 and closed its conservative mode-gate contract: startup clears it, Scene_EnterTransitionMode sets/clears it around transition service, and level-exit/player terminal paths consult it |
| `20260830-hud-display-nonzero-seen-static-v1` | recorded-static-decompilation | Named HUD_DISPLAY_NONZERO_SEEN at FFF0FE and closed the renderer's leading-zero contract: Render_BuildActorRecords clears it, skips leading ASCII zeroes, sets it on the first nonzero digit, and emits subsequent digits |
| `20260830-hud-interaction-frame-cursor-static-v1` | recorded-static-decompilation | Named HUD_INTERACTION_FRAME_CURSOR at FF7DA8 and closed its exact table-cursor contract: startup seeds HUD_INTERACTION_FRAME_SEQUENCE, Render_BuildActorRecords advances one word per interaction/HUD record, and the zero sentinel wraps to the post-default entry |
| `20260830-interaction-resource-milestones-static-v1` | recorded-static-decompilation | Named INTERACTION_RESOURCE_MODE_1B_MILESTONE and INTERACTION_RESOURCE_MODE_1C_MILESTONE at FF7E12/FF7E16; Level_LoadFromSceneState selects their ASCII thresholds by difficulty and Interaction_UpdateResourceState dispatches the exact mode-1B/1C transitions |
| `20260830-interaction-row-pointer-static-v1` | recorded-static-decompilation | Promoted INTERACTION_ROW_POINTER at FF7DAC and closed its camera/interaction contract: camera refill paths publish the selected terrain row, while the vertical and horizontal interaction processors consume it with INTERACTION_ROW_STRIDE |
| `20260830-interaction-coordinate-scratch-static-v1` | recorded-static-decompilation | Named INTERACTION_HANDLER_X/Y at FF7DB0/FF7DB2 and INTERACTION_SPAWN_X/Y_OFFSET at FFF150/FFF152, closing the row-processor coordinate publication and generic-spawn offset contract |
| `20260830-actor-type42-collision-step-static-v1` | recorded-static-decompilation | Named ACTOR_TYPE42_COLLISION_STEP at FFF10A and closed its exact four-contact contract: ActorType42_PlayerCollisionHandler advances the byte for each nonterminal collision, advances INTERACTION_COUNTER_DIGITS after step four, then clears the byte and reinitializes the actor |
| `20260830-player-terrain-transition-gate-static-v1` | recorded-static-decompilation | Named PLAYER_TERRAIN_TRANSITION_GATE at FFF114 and closed the one-shot terrain-transition contract: ActorType7E clears it when upward push ends, while the mode-11/mode-16 scene-resource actor helpers set it after publishing transition actors |
| `20260830-actor-vm-domain-selector-static-v1` | recorded-static-decompilation | Named ACTOR_VM_MOVEMENT_PASS at FF7DA2 and closed the shared actor-VM domain contract: AnimationVM clears it, MovementVM sets it to 0xFF, and cursor/timer/loop opcodes select animation versus movement actor fields from its zero/nonzero value |
| `20260830-actor-vm-continuations-static-v1` | recorded-static-decompilation | Named ACTOR_VM_COMMAND_CONTINUATION and ACTOR_VM_CURSOR_CLEAR_CONTINUATION at FF7D9A/FF7D9E and closed their pass-tail contract: AnimationVM installs 0x001AC842/0x001AC846, MovementVM installs 0x001AE048/0x001AE04C, and shared timer/loop or cursor-clear opcodes rewrite the saved return address through the matching pointer |
...
| `20260830-actor-render-camera-offsets-static-v1` | recorded-static-decompilation | Named ACTOR_RENDER_X_OFFSET and ACTOR_RENDER_Y_OFFSET at FFF080/FFF082 from the exact renderer arithmetic and Level-07/Levels-05/06 camera-callback writers |
| `20260830-actor-movement-origin-static-v1` | recorded-static-decompilation | Named ACTOR_MOVEMENT_ORIGIN_X and ACTOR_MOVEMENT_ORIGIN_Y at FFF090/FFF092 from MovementVM's pre-movement snapshots and post-movement delta propagation |
| `20260830-semantic-state-latches-static-v1` | recorded-static-decompilation | Promoted twelve directly bounded collision, presentation, menu, terrain, scene-resource, and Level-09 state fields from exact producer/consumer instruction contracts; retained ambiguous menu/debug bytes for later review |
| `20260830-actor-slot-type3d-camera-state-static-v1` | recorded-static-decompilation | Named the Type-0x3D interaction latch, shared normal-camera VDP render offset, and first common actor-pool record coordinate words from exact static producer/consumer contracts |
| `20260830-player-actor-options-gate-static-v1` | recorded-static-decompilation | Named the player actor record's absolute X/Y aliases and options-screen presentation gate from exact coordinate-publication and menu phase-selection contracts |
| `20260830-actor-slot-one-fields-static-v1` | recorded-static-decompilation | Completed the high-value absolute aliases for common actor-table slot 1: behavior/state flags, orientation, frame pointer, and animation-stream pointer |
| `20260830-actor-slot-two-transition-option-static-v1` | recorded-static-decompilation | Completed the high-value absolute aliases for fixed actor-table slot 2 and named the fourth scene-transition menu option with a neutral storage/behavior identity |
| `20260830-input-edge-menu-subphase-static-v1` | recorded-static-decompilation | Named the shared menu/sound-test input edge latch and bounded secondary options-menu phase counter with neutral role-based identities |
| `20260830-collision-terrain-latches-static-v1` | recorded-static-decompilation | Named the player collision X bounds, collision-to-interaction spawn gates, Type-11 surface-interaction block, action-animation state marker, and Level-04 event latches from exact producer/consumer instruction contracts; retained unresolved indirect producers explicitly |
| `20260830-interaction-resource-completion-latches-static-v1` | recorded-static-decompilation | Named the interaction-resource completion latch and finalization gate from the terminal progress branch in Interaction_UpdateResourceState, retaining their unresolved external consumer/producer edges |
| `20260830-fixed-actor-render-terrain-fields-static-v1` | recorded-static-decompilation | Named the fixed player/actor-slot views of actor sprite attribute, sprite VRAM base, vertical motion, and terrain-response fields from the shared actor-record initializer, renderer, and terrain dispatcher contracts |
| `20260830-scene-script-refresh-menu-latches-static-v1` | recorded-static-decompilation | Named the detected 50/60 Hz refresh selector, scene-script type-2 wait latch, and one-shot options presentation/audio latch from exact startup and writer/reader contracts |
| `20260830-actor-slot-movement-pointers-static-v1` | recorded-static-decompilation | Named fixed slot-one and slot-two actor movement-stream pointers from the shared template initializer and scene reset/instantiation contracts |
| `20260830-player-actor-state-fields-static-v1` | recorded-static-decompilation | Named player actor behavior and state flag bytes at actor-record offsets +0x06/+0x07 from renderer, VM, and transition-release access contracts |
| `20260830-actor-slot-status-flags-static-v1` | recorded-static-decompilation | Named fixed slot-one and slot-two actor status/lifecycle flag bytes at offset +0x3C from terrain, actor-VM callback, and retirement contracts |
| `20260830-actor-slot-three-four-scene-setup-static-v1` | recorded-static-decompilation | Named fixed actor-table slots 3 and 4 plus their X/Y coordinate words from Level12_ExitRoutine's exact template destinations and literal writes; slot 3 also anchors the common allocator's first non-player record |
| `20260830-actor-slots-five-seven-scene-rebuild-static-v1` | recorded-static-decompilation | Named fixed actor-table slots 5 through 7 and their X/Y/movement fields from exact scene-resource rebuild writes; slots 6 and 7 are additionally confirmed as the counter-selected interaction actors |
| `20260830-actor-slots-five-seven-animation-cursors-static-v1` | recorded-static-decompilation | Named the +0x20 animation-stream cursor fields for fixed actor-table slots 5 through 7; slots 6 and 7 are directly overwritten by the quotient/remainder-selected interaction animation roots |
| `20260830-player-actor-facing-y-flip-static-v1` | recorded-static-decompilation | Named PLAYER_ACTOR_FACING_Y_FLIP at FF7E75 from the renderer's exact sprite-attribute bit mapping and the shared VM toggle/copy contract |
| `20260830-player-collision-proximity-response-semantic-v1` | recorded-semantic-promotion | Promoted the runtime-observed Type-0x1E player-collision entry at 0x001AE796 to PlayerCollision_HandleProximityResponse, covering its Type-0x1E/0x21/0x1F/0x22 response group while retaining the type-only name as an alias |
| `20260830-player-collision-trampolines-semantic-v1` | recorded-semantic-promotion | Promoted the compact Type-0x7B and Type-0x06/0x0F player-collision entries to behavior-based trampoline names, retaining their type-only labels as aliases |
| `20260830-actor-terminal-interaction-semantic-v1` | recorded-semantic-promotion | Promoted the actor Type-0A terminal interaction entry at 0x001AC458 to ActorCollision_HandleTerminalInteraction, covering source cleanup, Type-2D interaction handling, and shared terminal response while retaining the type-only label as an alias |
| `20260830-player-collision-response-cluster-semantic-v1` | recorded-semantic-promotion | Promoted the Type-0x13 vertical response allocator, Type-0x1E recovery-plane wrapper, and shared Type-0x33/0x38/0x39 response-template path to behavior-based names while retaining their type-only labels as aliases |
| `20260830-actor-collision-response-cluster-semantic-v1` | recorded-semantic-promotion | Promoted the Type-0x13 staged actor response, Type-0x11 wall/terminal response, and Type-0x18/0x19 player interaction/proximity response to behavior-based names while retaining their type-only labels as aliases |
| `20260830-collision-transition-cluster-semantic-v1` | recorded-semantic-promotion | Promoted the Type-0x04 death-response, Type-0x2C/0x2D action-response, and Type-0x7E scene-transition collision paths to behavior-based names while retaining their type-only labels as aliases |
| `20260830-compact-player-response-family-semantic-v1` | recorded-semantic-promotion | Promoted the compact Type-0x37/0x3C counted response, Type-0x3D gated response, Type-0x41 wall response, and Type-0x34 wall replacement paths to behavior-based names while retaining their type-only labels as aliases |
| `20260830-secondary-player-response-family-semantic-v1` | recorded-semantic-promotion | Promoted the Type-0x2F action-response staging, Type-0x3A secondary counter response, and Type-0x11/0x12 interaction/launch response paths to behavior-based names while retaining their type-only labels as aliases |
| `20260830-counter-and-interaction-response-family-semantic-v1` | recorded-semantic-promotion | Promoted the Type-0x20 interaction response, Type-0x42 primary-counter response, and Type-0x29 response allocator to behavior-based names while retaining their type-only labels as aliases |
| `20260830-player-response-difficulty-family-semantic-v1` | recorded-semantic-promotion | Promoted the Type-0x43 interaction activation and Type-0x44/0x45 response-counter paths to behavior-based names while retaining their type-only labels as aliases |
| `20260830-player-transition-alignment-family-semantic-v1` | recorded-semantic-promotion | Promoted the Type-0x6E through 0x73 terrain alignment path and Type-0x4F transition-bounce response to behavior-based names while retaining their type-only aliases |
| `20260830-player-type1b-1c-death-response-semantic-v1` | recorded-semantic-promotion | Promoted the Type-0x1B and Type-0x1C gated death-response paths to behavior-based names while retaining their type-only aliases |
| `20260830-player-collision-latch-family-semantic-v1` | recorded-semantic-promotion | Promoted the Type-0x47 through Type-0x4C player-collision latch wrappers to behavior-based names and canonicalized the Type-0x4B latch while retaining dispatch-type aliases |
| `20260830-player-level-exit-response-semantic-v1` | recorded-semantic-promotion | Promoted the shared Type-0x74/0x75 player-collision level-exit response path to a behavior-based name while retaining its type-only alias |
| `20260830-compact-actor-player-response-semantic-v1` | recorded-semantic-promotion | Promoted the Type-0x14 interaction cleanup, Type-0x2B Type-0x84 event staging, and Type-0x30 player response reinitialization paths to behavior-based names while retaining dispatch-type aliases |
| `20260830-compact-actor-response-family-semantic-v1` | recorded-semantic-promotion | Promoted the compact Type-0x24/0x25/0x27/0x28 and Type-0x30 actor-collision response entries to behavior-based names while retaining dispatch-type aliases |
...
| `20260830-player-followup-response-family-semantic-v1` | recorded-semantic-promotion | Promoted Type-0x02 ground contact, Type-0x0C response reinitialization, Type-0x46 difficulty advancement, Type-0x54 terminal transition, and Type-0x1B/0x1C interaction entries while retaining dispatch-type aliases |
| `20260830-player-position-response-family-semantic-v1` | recorded-semantic-promotion | Promoted Type-0x5E actor-position adoption and Type-0x01 proximity-bounce paths while retaining dispatch-type aliases |
| `20260830-actor-collision-response-family-semantic-v1` | recorded-semantic-promotion | Promoted Type-0x02/0x08/0x09 cleanup, Type-0x23 response-pair start, Type-0x10 staged response, and Type-0x2F cleanup entries while retaining dispatch-type aliases |
| `20260830-player-terrain-exit-family-semantic-v1` | recorded-semantic-promotion | Promoted Type-0x08/0x09 directional terrain push, Type-0x4D paired response, and Type-0x60/0x61 exit-presentation paths while retaining dispatch-type aliases |
| `20260830-player-contact-state-family-semantic-v1` | recorded-semantic-promotion | Promoted Type-0x69/0x6A handhold contact, Type-0x32/0x79 interaction-state entry, and Type-0x50/0x51 transition contact while retaining dispatch-type aliases |
| `20260830-player-surface-bounce-family-semantic-v1` | recorded-semantic-promotion | Promoted Type-0x65 bounce response and Type-0x5A/0x5B surface-mode entries while retaining dispatch-type aliases |
| `20260830-player-interaction-family-semantic-v1` | recorded-semantic-promotion | Promoted Type-0x1A actor interaction and Type-0x7D/0x78/0x7A player interaction-proximity paths while retaining dispatch-type aliases |
...
| `20260831-remaining-data-identities-semantic-v1` | recorded-semantic-promotion | Replaced address-bearing names for the last three structurally decoded data objects with stable resource identities while retaining legacy aliases |
| `20260831-scene-resource-graphics-semantic-v1` | recorded-semantic-promotion | Replaced address-bearing names for four RNC graphics resources with stable scene-state and VRAM-destination identities while retaining legacy aliases |
| `20260831-rebuild-active-graphics-semantic-v1` | recorded-semantic-promotion | Replaced address-bearing names for the rebuild and active state-01 RNC graphics pairs with stable VRAM-destination identities while retaining legacy aliases |
| `20260831-presentation-graphics-semantic-v1` | recorded-semantic-promotion | Replaced address-bearing names for active-scene, state-presentation, and script-transition RNC graphics resources with stable consumer and VRAM-destination identities while retaining legacy aliases |
| `20260831-title-menu-graphics-semantic-v1` | recorded-semantic-promotion | Replaced address-bearing names for the wish-prompt resource and title E000/C000 graphics sequence with stable consumer and VRAM-destination identities while retaining legacy aliases |
| `20260831-shared-base-graphics-semantic-v1` | recorded-semantic-promotion | Replaced address-bearing names for four large shared/base RNC graphics payloads with stable destination and consumer-family identities while retaining legacy aliases |
| `20260831-scene-presentation-streams-semantic-v1` | recorded-semantic-promotion | Replaced address-bearing names for directly wrapped scene presentation and blank streams with stable state and order identities while retaining legacy aliases |
| `20260831-shared-scene-resources-semantic-v1` | recorded-semantic-promotion | Replaced address-bearing names for the directly consumed transition presentation stream and shared active/reset graphics payload with stable consumer identities while retaining legacy aliases |
| `20260831-scene-palette-consumer-identities-semantic-v1` | recorded-semantic-promotion | Replaced address-bearing names for eight directly assigned scene-resource palette sources with stable canonical graphics-consumer identities while retaining legacy aliases |
| `20260831-direct-palette-loader-identities-semantic-v1` | recorded-semantic-promotion | Replaced address-bearing names for seven directly loaded or published palette sources with stable menu, interaction, and scene-loader identities while retaining legacy aliases |
| `20260831-type84-scene-template-identities-semantic-v1` | recorded-semantic-promotion | Replaced address-bearing Type-0x84 scene-template and terminal-animation names with stable lifecycle identities while retaining legacy aliases |
| `20260831-scene-resource-coordinate-identities-semantic-v1` | recorded-semantic-promotion | Replaced the address-bearing name for the directly consumed queued-actor coordinate stream with a stable resource identity while retaining the legacy alias |
| `20260831-menu-palette-container-identities-semantic-v1` | recorded-semantic-promotion | Replaced address-bearing names for two menu-owned palette containers with stable menu identities while preserving unresolved internal transfer semantics and legacy aliases |
| `20260831-player-collision-type4b-latch-closure-v1` | recorded-negative-evidence | Closed the Type-0x4B handler/latch review item after the whole-ROM database confirmed zero static latch readers and no incoming handler references |
| `20260831-actor-vertical-motion-movement-writers-static-v1` | recorded-static-decompilation | Closed the producer-unknown description for ACTOR_SLOT_2_VERTICAL_MOTION: all 85 layout-bounded MovementVM streams were decoded and yielded one actor-relative 0x83 zero write plus 23 actor-relative 0x90 residual-motion adjustments |
| `20260831-vm-stream-semantic-naming-static-v1` | recorded-static-decompilation | Replaced the unreferenced/address-derived VM labels at 0x0011F800 and 0x00123DE2 with behavior-based identities for the interaction-anchor grid response and single-frame self-loop, retaining legacy aliases and unresolved external reachability |
| `20260831-interaction-spawn-handoff-marker-static-v1` | recorded-static-decompilation | Named the previously untracked 0x00FFF0E5 interaction-spawn handoff marker from two exact terminal MovementVM writes and nine direct clr.b sites in the Type-0x5E/Type-0x84 spawn family, retaining its unresolved direct reader |
| `20260831-level04-event38-latch-static-v1` | recorded-static-decompilation | Closed the exact Level04_EnterRoutine consumer and unconditional clear for LEVEL04_EVENT_38_PENDING at 0x001B5BC6 while retaining the unresolved upstream setter |
| `20260831-player-collision-type18-19-gate-reset-static-v1` | recorded-static-decompilation | Closed the startup reset contract for PLAYER_COLLISION_TYPE18_19_PAIR_SPAWN_GATE: System_ResetBootstrap zero-fills the full work-RAM window through 0x00000276 while the later post-set clear remains unresolved |
| `20260831-interaction-resource-completion-marker-static-v1` | recorded-static-decompilation | Refined INTERACTION_RESOURCE_COMPLETION_LATCH to INTERACTION_RESOURCE_COMPLETION_MARKER after confirming the exact terminal st write at 0x001B0290 has no direct read or clear, preserving the legacy alias |
| `20260831-unresolved-vm-streams-runtime-negative-v1` | recorded-negative-evidence | Added bounded natural-route evidence that neither the interaction-anchor grid MovementVM stream at 0x0011F800 nor the single-frame AnimationVM self-loop at 0x00123DE2 is read during the verified 2,296-frame Level 01 route; external reachability remains open |
| `20260831-actor-vm-orphan-identities-semantic-v1` | recorded-static-decompilation | Promoted the standalone 0x001203E8 MovementVM drift loop and the two unreferenced Type-0x84 template records at 0x001B7990/0x001B7A58 to behavior/field-based identities, retaining legacy aliases and unresolved reachability |
| `20260831-actor-vm-orphans-natural-route-runtime-negative-v1` | recorded-negative-evidence | Batched all five standalone actor VM/template ranges into the verified 2,296-frame Level 01 route and observed zero ROM reads, closing only route-specific reachability while leaving multi-scene and indirect-selector paths open |
| `20260831-actor-vm-orphans-long-route-runtime-negative-v1` | recorded-negative-evidence | Extended the same five-range taps through the 5,000-frame long-gameplay traversal with zero reads; the run stayed in scene state 1, so further reachability work must use another scene or indirect-selector path |
| `20260831-actor-vm-orphans-state08-runtime-negative-v1` | recorded-negative-evidence | Reproduced the ROM-owned transition to scene state 0x08 with the established table-index setup and observed zero reads for all five standalone actor VM/template ranges; remaining reachability work is indirect selector/table analysis |
| `20260831-actor-vm-orphans-transition-checkpoints-runtime-negative-v1` | recorded-negative-evidence | Extended the four remaining actor VM/template review ranges across six saved scene-1 and scene-3 transition checkpoints with zero reads; their behavior/field names remain conservative pending indirect selector/table evidence |
| `20260831-scene-resource-setup-loader-identities-semantic-v1` | recorded-semantic-promotion | Replaced source-address-bearing names for four setup-time scene-resource loaders with stable graphics-role and VRAM-destination identities while retaining legacy aliases |
| `20260831-scene-resource-variant-loader-identities-semantic-v1` | recorded-semantic-promotion | Replaced palette-address-bearing names for four compact scene-resource variant loaders with stable graphics-role, palette, and dispatch-state identities while retaining legacy aliases |
| `20260831-scene-resource-c000-loader-identities-semantic-v1` | recorded-semantic-promotion | Replaced source-address-bearing names for five state-specific C000 scene-resource loaders with stable scene-state, graphics, palette, and render-frame identities while retaining legacy aliases |
| `20260831-scene-resource-presentation-wrapper-identities-semantic-v1` | recorded-semantic-promotion | Replaced source-address-bearing names for nine scene presentation wrappers and their shared C000 loader with stable state, resource, stream, palette, and parameter identities while retaining legacy aliases |
| `20260831-scene-resource-blank-wrapper-identities-semantic-v1` | recorded-semantic-promotion | Replaced stream-address-bearing names for eight blank presentation wrappers with stable state, stream, and parameter identities while retaining legacy aliases |
| `20260831-camera-scroll-callback-identities-semantic-v1` | recorded-semantic-promotion | Replaced ROM-offset-bearing names for the three AnimationVM camera-scroll callbacks with stable full, reduced, and tail profile identities while retaining legacy aliases |
| `20260831-actor-type84-collision-transition-identity-semantic-v1` | recorded-semantic-promotion | Replaced the animation-address-bearing shared actor transition name at 0x001AC0D2 with its exact Type-0x84 collision-response role while retaining the legacy alias |
| `20260831-player-collision-animation-prelude-static-v1` | recorded-semantic-promotion | Replaced the generic animation-stream label at 0x001220AA with the producer-backed player collision-response prelude selected by Actor_PlayerCollisionPass, including its exact 0x095E/F4/F8 behavior |
| `20260831-player-action-terrain-timer-continuation-static-v1` | recorded-semantic-promotion | Replaced the generic animation-stream label at 0x00122910 with the exact interaction-flag continuation of PLAYER_ANIM_ACTION_TERRAIN_TIMER_GROUNDED, including its terrain gates, response writes, sound, and Type-0x84 spawn branch |
| `20260831-terrain-response-auxiliary-flag-static-v1` | recorded-semantic-promotion | Replaced generated Unknown_00FFF0E4 with the conservative terrain-response auxiliary flag set by behavior 0x10, retaining the unresolved-reader limitation |
| `20260831-player-collision-current-actor-type-static-v1` | recorded-semantic-promotion | Replaced generated Unknown_00FFF0F6 with the transient actor type published by Actor_PlayerCollisionPass before handler dispatch, retaining the unresolved-reader limitation |
| `20260831-level-event-presentation-state-static-v1` | recorded-semantic-promotion | Replaced generated Unknown_00FFF10B with the level-event presentation selector written by command 0xF3 and consumed by the Level-08 exit presentation stream for its zero/one directional phase |
| `20260831-scene-resource-service-marker-static-v1` | recorded-semantic-promotion | Replaced generated Unknown_00FFF158 with the shared scene/resource service marker toggled around frame VDP/palette servicing and interaction-resource mode handoffs, retaining the unresolved-reader limitation |
| `20260831-actor-table-slot-24-base-static-v1` | recorded-semantic-promotion | Replaced generated Unknown_00FF8470 with the fixed actor-table slot-24 base proven by the 0x42-byte record stride and backward allocator's slots-24-through-1 scan |
| `20260831-menu-controller-poll-gate-static-v1` | recorded-semantic-promotion | Replaced generated Unknown_00FFEFFE with the menu-local controller-poll gate proven by its set/clear bracket around the controller-port handshake and its two selection-flow tests |
| `20260831-render-sprite-record-buffer-static-v1` | recorded-semantic-promotion | Replaced generated Unknown_00FF729A and Unknown_00FFEFEC with the shared four-word VDP sprite-record buffer and its terminator-inclusive published count |
| `20260831-vdp-tile-row-command-tables-static-v1` | recorded-semantic-promotion | Replaced generated Unknown_00FF8680, Unknown_00FF8700, and Unknown_00FF8780 with the selected, selected-mirror, and alternate 32-entry VDP tile-row command tables |
| `20260831-actor-sprite-payload-buffer-static-v1` | recorded-semantic-promotion | Replaced generated Unknown_00FF769A with the actor-sprite payload buffer shared by AnimationVM payload construction and VDP submission |
| `20260831-actor-table-slot-20-base-static-v1` | recorded-semantic-promotion | Replaced generated Unknown_00FF8368 with the fixed actor-table slot-20 base proven by the reverse-20 allocator and interaction-spawn pool |
| `20260831-interaction-response-flag-static-v1` | recorded-semantic-promotion | Replaced generated Unknown_00FFF104 with the interaction-response flag cleared by the Type-0x80/0x87 reverse-slot spawn handler, retaining the unresolved-reader limitation |
| `20260831-game-startup-stack-slot-negative-v1` | recorded-negative-evidence | Closed DAT_00FFEFD4 as startup stack bookkeeping: SP is initialized at FFEFD8, call returns use FFEFD4, and the sole read is a nested MOVEM spill-slot access |
| `20260831-terrain-response-auxiliary-flag-setter-semantic-v1` | recorded-semantic-promotion | Replaced the address-qualified terrain behavior-0x10 setter name with TerrainHandler_SetAuxiliaryResponseFlag, retaining TerrainHandler_SetResponseFlag_FFF0E4 as a legacy alias and leaving downstream meaning unresolved |
| `20260831-interaction-spawn-type40-helper-semantic-v1` | recorded-semantic-promotion | Replaced the runtime-type-only InteractionSpawn_RuntimeType40 helper name with InteractionSpawn_CreateType40Variant, retaining the legacy alias and exact selector-0x61 four-position spawn contract |
| `20260831-interaction-spawn-type6e-73-semantic-v1` | recorded-semantic-promotion | Replaced six runtime-type-only interaction handlers with stable CreateType...Actor identities, retaining legacy aliases and the exact shared allocation/template/coordinate contract with per-type MovementVM roots |
| `20260831-interaction-spawn-type3c-3e-3f-response-semantic-v1` | recorded-semantic-promotion | Replaced three selector-qualified runtime-type interaction names with stable type-specific response-spawn identities, retaining aliases and the exact gate, forward-allocation, shared animation, and MovementVM contracts |
| `20260831-interaction-spawn-type17-palette-semantic-v1` | recorded-semantic-promotion | Replaced the five-selector runtime-type interaction name with InteractionSpawn_CreateType17WithPalette, retaining the legacy alias and exact Type-0x17 template, camera gate, and palette-upload contract |
| `20260831-interaction-spawn-response-family-semantic-v1` | recorded-semantic-promotion | Replaced four late interaction runtime-type/selector names with stable Type-0x87, Type-0x3A, Type-0x3B, and Type-0x41 response-creation identities, retaining aliases and exact gate/template/animation/state contracts |
| `20260831-interaction-spawn-type47-4c-semantic-v1` | recorded-semantic-promotion | Replaced six selector-qualified runtime-type names with stable Type-0x47 through Type-0x4C actor-creation identities, retaining aliases and the shared zero-template, animation, movement-clear, actor-flag, allocator, and Type-0x4A latch contracts |
| `20260831-interaction-spawn-type20-21-22-semantic-v1` | recorded-semantic-promotion | Replaced three upper-route runtime-type/selector names with stable Type-0x20 upper, Type-0x21 proximity, and Type-0x22 proximity actor-creation identities, retaining aliases and exact template, allocator, runtime-type, and AnimationVM contracts |
| `20260831-interaction-spawn-type52-semantic-v1` | recorded-semantic-promotion | Replaced seven Type-0x52 runtime-type/selector names with stable difficulty and presentation variant creation identities, retaining aliases and exact shared template, reverse-slot allocator, difficulty gate, and AnimationVM contracts |
| `20260831-interaction-spawn-type37-43-8a-84-34-semantic-v1` | recorded-semantic-promotion | Replaced five adjacent runtime-type/selector names with stable Type-0x37 response, Type-0x43 base, Type-0x8A Type-0x43-derived, Type-0x84 interaction, and Type-0x34 wall actor-creation identities, retaining aliases and exact asset/allocation contracts |
| `20260831-interaction-spawn-type40-semantic-v1` | recorded-semantic-promotion | Replaced the Type-0x40 selector-0x61 four-position, selector-0xAD gate, and selector-0x60 single-actor names with stable creation identities, retaining aliases and exact counter, gate, helper, template, animation, and coordinate contracts |
| `20260831-interaction-spawn-late-compact-semantic-v1` | recorded-semantic-promotion | Replaced eleven compact late interaction runtime-type/selector names with stable Type-0x44, Type-0x5A/0x5C/0x5D, Type-0x01, Type-0x4E/0x4F, Type-0x53, and Type-0x84 creation identities, retaining aliases and exact template, gate, allocator, facing, animation, and coordinate contracts |
| `20260831-interaction-spawn-unindexed-variants-semantic-v1` | recorded-semantic-promotion | Replaced three complete unindexed adjacent-variant names with stable Type-0x57-from-Type-0x55, Type-0x45, and Type-0x5B-from-Type-0x5A creation identities, retaining aliases and explicit unresolved selector/reachability status |
| `20260831-interaction-spawn-type84-base-semantic-v1` | recorded-semantic-promotion | Replaced six Type-0x84 interaction-base selector names with stable creation identities, retaining aliases and exact gate, shared-template, reverse-slot, animation-root, and Y-adjustment contracts |
| `20260831-interaction-spawn-early-family-semantic-v1` | recorded-semantic-promotion | Replaced 21 early interaction dispatch names with stable Type-0x84/0x62/0x64/0x1A/0x1B/0x1C/0x58/0x55/0x74/0x07/0x8C/0x76 creation and gate identities, retaining legacy aliases and exact static contracts |
| `20260831-interaction-spawn-mid-family-semantic-v1` | recorded-semantic-promotion | Replaced 11 mid interaction dispatch names with stable Type-0x89/0x6A/0x69/0x6C/0x23/0x06/0x2B/0x84/0x5F creation and gate identities, retaining legacy aliases and exact static contracts |
| `20260831-interaction-spawn-type5e84-pair-semantic-v1` | recorded-semantic-promotion | Replaced seven active Type-0x5E/0x84 paired-spawn dispatch names with stable creation identities, retaining legacy aliases and exact MovementVM stream, pair-link, companion-anchor, offset, and gate contracts |
| `20260831-interaction-spawn-type5e-threshold-semantic-v1` | recorded-semantic-promotion | Replaced four Type-0x5E threshold and Type-0x29/0x67 interaction dispatch names with stable creation identities, retaining legacy aliases and exact threshold, cleanup, template, selector, and allocator contracts |
| `20260831-interaction-spawn-response-family-static-semantic-v1` | recorded-semantic-promotion | Replaced six Type-0x05/0x03/0x10/0x2F response-spawn dispatch names with stable creation identities, retaining legacy aliases and exact selector, template, palette, audio, allocator, and facing contracts |
| `20260831-interaction-spawn-type11-12-semantic-v1` | recorded-semantic-promotion | Replaced the Type-0x11 palette and Type-0x12 presentation interaction names with stable creation identities, retaining legacy aliases and exact template, allocator, palette, presentation-flag, and audio contracts |
| `20260831-interaction-spawn-type8b-presentation-semantic-v1` | recorded-semantic-promotion | Replaced four Type-0x8B presentation interaction names with stable creation identities, retaining legacy aliases and exact shared-template, backward-slot, animation-root, palette, and gate contracts |
| `20260831-interaction-spawn-upper-gated-family-semantic-v1` | recorded-semantic-promotion | Replaced four upper-route Type-0x20/0x1E/0x1F interaction names with stable creation and gate identities, retaining legacy aliases and exact template, allocator, and gate contracts |
| `20260831-interaction-spawn-type79-type7a-semantic-v1` | recorded-semantic-promotion | Replaced four Type-0x79/0x7A scene-interaction names with stable creation identities, retaining legacy aliases and exact template, allocator, selector, and AnimationVM-root contracts |
| `20260831-interaction-spawn-late-level-object-family-semantic-v1` | recorded-semantic-promotion | Replaced seven late level-object and interaction names with stable guard, level-event, Type-0x0F, Type-0x16, and Type-0x36 creation identities, retaining legacy aliases and exact selector, template, allocator, gate, palette, and direct side-effect contracts |
| `20260831-interaction-spawn-type13-type14-semantic-v1` | recorded-semantic-promotion | Replaced the selector-0x28 Type-0x13 and selector-0x13 Type-0x14 interaction names with stable creation identities, retaining legacy aliases and exact template, allocator, gate, palette, audio, and AnimationVM-root contracts |
| `20260831-interaction-spawn-type1d-type1e-semantic-v1` | recorded-semantic-promotion | Replaced the confirmed selector-0x65 Type-0x1D gate and selector-0x11 direct Type-0x1E interaction names with stable identities, retaining legacy aliases and exact handler-table, gate, template, allocator, and upper-route dispatch contracts |
| `20260831-interaction-spawn-type46-gate-semantic-v1` | recorded-semantic-promotion | Replaced the selector-0xAF Type-0x46 interaction gate name with a stable creation identity, retaining the legacy alias and exact generic-gate, difficulty-counter, shared-template, and forward-allocator contract |
| `20260831-actor-collision-type23-pair-semantic-v1` | recorded-semantic-promotion | Replaced the Type-0x23 collision-response pair builder and zero-template/source-position helper names with stable actor-collision identities, retaining legacy aliases and exact source conversion, companion allocation, template, animation, and position-copy contracts |
| `20260831-actor-collision-shared-reinitializer-semantic-v1` | recorded-semantic-promotion | Replaced the shared interaction reinitializer and terminal-response tail names with stable actor-collision service identities, retaining legacy aliases and exact template selection, response-counter, countdown, cleanup, and audio contracts |
| `20260831-semantic-review-closure-static-v1` | recorded-negative-evidence | Closed the remaining named-but-open static review items for the three unindexed interaction spawners, four orphan VM/template ranges, and the published player-collision actor-type byte while preserving their unresolved indirect selector/consumer limitations |
| `20260831-terrain-response-auxiliary-flag-closure-v1` | recorded-negative-evidence | Closed TERRAIN_RESPONSE_AUXILIARY_FLAG as an unconsumed static terrain-response field after the whole-ROM database confirmed one writer, zero direct readers, and no runtime observations, while preserving the indirect-reader limitation |
| `20260831-level-table-callback-identities-semantic-v1` | recorded-semantic-promotion | Renamed all 26 LEVEL_TABLE per-frame and exit callback entry points from EnterRoutine/ExitRoutine to phase-accurate LevelXX_FrameCallback/LevelXX_ExitCallback identities while retaining every historical name as an alias |
| `20260831-terrain-handler-identities-semantic-v1` | recorded-semantic-promotion | Replaced 11 generic terrain response Block labels with action-accurate Set, Apply, Push, Snap, Launch, Bounce, and surface-mode identities while retaining every historical label as an alias |
| `20260831-shared-collision-and-vram-services-semantic-v1` | recorded-semantic-promotion | Replaced the remaining generic shared player-collision Block/trampoline and fixed VRAM Block labels with exact service identities while retaining every historical label as an alias |
